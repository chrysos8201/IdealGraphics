...
void D3D12VertexBuffer::Create(ID3D12Device* Device, ID3D12Heap* Heap, uint32 Offset, ID3D12GraphicsCommandList* CmdList, uint32 ElementSize, uint32 ElementCount, const D3D12UploadBuffer& UploadBuffer)
{
	D3D12GPUBuffer::CreateBuffer(Device, Heap, Offset, ElementSize, ElementCount);
	D3D12GPUBuffer::SetName(L"VertexBuffer");
	// 데이터를 복사한다
	CmdList->CopyBufferRegion(m_resource.Get(), 0, UploadBuffer.GetResource(), 0, m_bufferSize);
